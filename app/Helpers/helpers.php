...
function getvendorRegistrationduration($vendorduration) {
    return $vendorduration->duration->duration;
}

function getvendorRegistrationdiscount($vendorduration) {
    return $vendorduration->duration->discount;
}
...
function getTransfererName($TransferTransaction) {
    $username = User::find($TransferTransaction->from_id)->name;
    return $username;
}

function getReceiverName($TransferTransaction) {
    $username = User::find($TransferTransaction->to_id)->name;
    return $username;
}


function getFullRating($review) {
    return ($review->trust_rating+$review->speed_rating+$review->knowledge_rating+$review->price_rating+$review->service_rating)/5;
}

function getBannedUsername($BannedUsers) {
    return $BannedUsers->user->name;
}

function getSubcategoryRelationName($subcategory) {
    return $subcategory->categories->name;
}

function getSubcategoryVendorRelationName($subcategory) {
...
function getsSpecialRelationName($special) {
    return $special->users->name;
}
function getVendorName($review) {
    return $review->vendors->name;
}
...
function getRating($vendor) {
    ...
    $sum = 0;
    $i=0;
    foreach ($vendor->clients as $review) {

        $sum += $review->pivot->price_rating+$review->pivot->service_rating+$review->pivot->trust_rating+$review->pivot->speed_rating+$review->pivot->knowledge_rating;
        $i++;
    }
    if($sum==0) return $sum;
    else return $sum/($i*5);
...
function myReviewRating($attr) {
    $sum = 0;
    $sum += $attr->pivot->price_rating + $attr->pivot->service_rating + $attr->pivot->trust_rating + $attr->pivot->speed_rating + $attr->pivot->knowledge_rating;
    if ($sum == 0) return $sum = 0;
    else return $sum = $sum / 5;
}
...
function getApprovedColumn($modelObject, $attributeName = 'approved')
{
    ...
    {
        return "<span class='text-info'>Approved</span>";
    }
    elseif($modelObject[$attributeName] == false)
    {
...
function getLanguageColumn($column, $attributeName)
{
    if (isset($column)) {
        if ($column[$attributeName]) {
            return "<a class='btn btn-outline-secondary btn-sm' href='mailto:" . $column[$attributeName] . "'><i class='fa fa-envelope mr-1'></i>" . $column[$attributeName] . "</a>";
...
function getPickupOrDeliveryColumn($modelObject,$attributeName = 'pickupOrDelivery')
{
    if (isset($modelObject)) {
        if ($modelObject[$attributeName]) {
            return "<span class='badge badge-success'>" . trans('lang.delivery') . "</span>";
        } else {
            return "<span class='badge badge-danger'>" . trans('lang.pickup') . "</span>";
        }
    }
}

function getTimeColumn($modelObject,$attributeName = 'time')
{
    return "<span>$modelObject[$attributeName]</span>";
}

function getDeliverDateColumn($modelObject,$attributeName = 'date')
{
    return "<span>$modelObject[$attributeName]</span>";
}
...
function getOrder_idColumn($modelObject,$attributeName = 'order_id')
{
    return "<span>$modelObject[$attributeName]</span>";
}

function getuser_idColumn($modelObject,$attributeName = 'user_id')
{
    return "<span>$modelObject[$attributeName]</span>";
}

function getMsgColumn($modelObject,$attributeName = 'msg')
{
    //$modelObject[$attributeName]
    return "<span>$modelObject[$attributeName]</span>";
}
function getUserColumn($modelObject,$attributeName = 'user_id')
{
    return "<span>$modelObject[$attributeName]</span>";
}
function getRestaurantColumn($modelObject,$attributeName = 'restaurant_id')
{
    return "<span>$modelObject[$attributeName]</span>";
}

...
class MessagesController extends Controller
{
    ...
    {
        $path = asset() . '/' . config('chatify.attachments.folder') . '/' . $fileName;
        if (file_exists($path)) {
    ...
    public function send(Request $request)
    {
        ...
        //  if (input::post('VoiceMessage')) {
        // //     $VoiceMessage = input::post(VoiceMessage);
        // //     $file->storeAs("/".config('chatify.attachments.folder'), $VoiceMessage);
        //         echo 'ss';
        //  }
        ...
        if (!$error_msg) {
            // send to database
            $messageID = mt_rand(9, 9999999) + time();
            Chatify::newMessage([
                'id' => $messageID,
                'type' => $request['type'],
            ...
            if ($file->getSize() < 150000000) {
                if (in_array($file->getClientOriginalExtension(), $allowed_images)) {
                    ...
                    if (Auth::user()->avatar != config('chatify.user_avatar.default')) {
                        $path = public_path('/' . config('chatify.user_avatar.folder') . '/' . Auth::user()->avatar);
                        if (file_exists($path)) {
                            @unlink($path);
                        }
                    }

...
class OrderAPIController extends Controller
{
    ...
    public function Kitchen_confirmed_order(Request $request)
    {
        $order_id = $request->order_id;
        
        try{
            $order = Order::find($order_id);
            $order->confirmed_byRestaurant = 1;
            $order->save();
        }
        ...
        return $this->sendResponse($order->toArray(), 'Order confirmed successfully');
    }
    public function timer_30_mints()
    {
        sleep(10);
        echo " 10 second";
    }
            ...
            if ($stripeToken->created > 0) {
                if (empty($input['delivery_address_id'])) {
                    $order = $this->orderRepository->create(
                        $request->only('user_id', 'order_status_id', 'tax', 'hint','date','time','pickupOrDelivery')
                    );
                } else {
                    $order = $this->orderRepository->create(
                        $request->only('user_id', 'order_status_id', 'tax', 'delivery_address_id', 'delivery_fee', 'hint','date','time','pickupOrDelivery')
                    );
                }
